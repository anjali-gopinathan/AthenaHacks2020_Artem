...
void updateHeartRate() {
  ...
  */
  ...
  if (heartRateMeasurement>=800){
    int heartRate = map(heartRateMeasurement, 0, 1300, 0, 120);
      if (heartRate != oldHeartRate) {      // if the heart rate has changed
        Serial.print("Heart Rate is now: "); // print it
        Serial.println(trueHeartRate(heartAvgs, heartRate, i));
...
int trueHeartRate(int heartAvgs[10], int heartRate, int & i){
  if (i<10){
    heartAvgs[i]=heartRate;
    i++;
  }
  else {
    heartAvgs[9]=heartRate;
    for (int j=9; j>0; j--){
      heartRate=heartRate+heartAvgs[j--];
    }
